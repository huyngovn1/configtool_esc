...
QString stepperText(int rawValue, double scale, double offset) {
    QString text = QString::number((rawValue + offset) * scale, 'f', 2);
...
// The existing sliders remain the single source of truth for EEPROM values.
// This wrapper only adds direct typing and +/- controls to the UI.
QFrame *makeStepper(QWidget *parent, const char *name, QSlider *slider,
                    QWidget *legacyValue, double scale = 1.0,
                    double offset = 0.0) {
    auto *stepper = parent->findChild<QFrame *>(name);
    if (!stepper) {
        stepper = new QFrame(parent);
        stepper->setObjectName(name);
        stepper->setProperty("webStepper", true);
        stepper->setFrameShape(QFrame::NoFrame);

        auto *minus = new QToolButton(stepper);
        minus->setObjectName("minus");
        minus->setText("-");
        minus->setProperty("webStepperButton", true);

        auto *plus = new QToolButton(stepper);
        plus->setObjectName("plus");
        plus->setText("+");
        plus->setProperty("webStepperButton", true);

        auto *editor = new QLineEdit(stepper);
        editor->setObjectName("webStepperEditor");
        editor->setProperty("webStepperValue", true);
        editor->setAlignment(Qt::AlignCenter);

        QObject::connect(minus, &QToolButton::clicked, slider, [slider] {
            slider->setValue(qMax(slider->minimum(),
                                  slider->value() - qMax(1, slider->singleStep())));
        });
        QObject::connect(plus, &QToolButton::clicked, slider, [slider] {
            slider->setValue(qMin(slider->maximum(),
                                  slider->value() + qMax(1, slider->singleStep())));
        });
        QObject::connect(slider, &QSlider::valueChanged, editor,
                         [editor, scale, offset](int value) {
                             editor->setText(stepperText(value, scale, offset));
                         });
        QObject::connect(editor, &QLineEdit::editingFinished, slider,
                         [editor, slider, scale, offset] {
                             bool valid = false;
                             const double entered = editor->text().trimmed().toDouble(&valid);
                             if (valid && scale != 0.0) {
                                 const int rawValue = qRound((entered / scale) - offset);
                                 slider->setValue(qBound(slider->minimum(), rawValue,
                                                         slider->maximum()));
                             }
                             editor->setText(stepperText(slider->value(), scale, offset));
                         });
        editor->setText(stepperText(slider->value(), scale, offset));
    }
    ...
    slider->hide();
    legacyValue->hide();
    stepper->setEnabled(slider->isEnabled());
    return stepper;
}

void placeStepper(QFrame *stepper, int x, int y, int width, int height) {
    if (!stepper) {
        return;
    }
    place(stepper, x, y, width, height);
    place(stepper->findChild<QToolButton *>("minus"), 0, 0, 31, height);
    place(stepper->findChild<QLineEdit *>("webStepperEditor"), 31, 0,
          qMax(24, width - 62), height);
    place(stepper->findChild<QToolButton *>("plus"), width - 31, 0, 31, height);
}
...
Widget::Widget(QWidget *parent)
    ...
    ui->setupUi(this);
    // Single-ESC workflow: retain only Settings, Flash and Input.
    ui->tabWidget->removeTab(5); // FUTURE
    ui->tabWidget->removeTab(4); // Startup Tune
    ui->tabWidget->removeTab(2); // Motor Control
    this->setWindowTitle("ESC Control Center");
    installWebSkin();
...
void Widget::installWebSkin() {
    ...
    ui->scrollAreaWidgetContents_5->setMinimumWidth(960);
    ...
    QLabel#webLogo {
      background: #377cf3; color: white; border: 1px solid #5a96ff;
    ...
    QPushButton#webDisconnectButton { color: #b42318; padding: 0 10px; font-size: 11px; }
    ...
    QFrame[webStepper="true"] {
      background: #fbfcff; border: 1px solid #cbd7e6; border-radius: 6px;
    }
    QFrame[webStepper="true"]:disabled { background: #f6f8fb; border-color: #e3e8f0; }
    QToolButton[webStepperButton="true"] {
      color: #123262; background: #f8faff; border: none; font-size: 15px; font-weight: 700;
    }
    QToolButton#minus { border-right: 1px solid #d9e1ed; border-top-left-radius: 5px; border-bottom-left-radius: 5px; }
    QToolButton#plus { border-left: 1px solid #d9e1ed; border-top-right-radius: 5px; border-bottom-right-radius: 5px; }
    QToolButton[webStepperButton="true"]:hover { background: #eaf1ff; }
    QLineEdit[webStepperValue="true"] { border: none; border-radius: 0; padding: 0 4px; background: #ffffff; text-align: center; }
    QLCDNumber[webStepperValue="true"] { border: none; border-radius: 0; background: #ffffff; }
    ...
    makeLabel(ui->frame_2, "webLogo", "EC");
    makeLabel(ui->frame_2, "webKicker", "ESC", "brandKicker");
    makeLabel(ui->frame_2, "webTitle", "ESC Control Center", "brandTitle");
    ...
    ui->ConnectedButton->setAttribute(Qt::WA_TransparentForMouseEvents, true);
    ui->ConnectedButton->setText("DEVICE   Not connected");
    // QLabel has no textChanged signal.  Polling the existing status labels keeps
    // the header in sync without changing any ESC serial protocol code.
    auto *deviceStatusTimer = new QTimer(this);
    connect(deviceStatusTimer, &QTimer::timeout, this, [this] {
        QString message;
        if (ui->tabWidget->currentWidget() == ui->tab_2) {
            message = ui->escStatusLabel_2->text();
        } else {
            message = ui->escStatusLabel->text();
        }
        ...
        const bool ready = m_serial && m_serial->isOpen() && four_way &&
                           four_way->ESC_connected;
        ui->ConnectedButton->setChecked(ready);
        ui->ConnectedButton->setText("DEVICE   " + message);
        ui->ConnectedButton->setToolTip(message);
    });
    deviceStatusTimer->start(100);

    makeCard(ui->frame, "webActionSurface");
    const char *captions[] = {"FIRMWARE", "EEPROM", "EEPROM ADDRESS", "FLASH CODE", "CONNECTION MODE"};
    const char *values[] = {"--", "--", "0x7C00", "0x1F", "USB / Wi-Fi"};
    ...
    makeStepper(ui->eepromFrame, "webTimingStepper", ui->timingAdvanceSlider,
                ui->timingAdvanceLCD, 0.9375, -10.0);
    makeStepper(ui->eepromFrame, "webKvStepper", ui->motorKVSlider,
                ui->motorKVLCD, 40.0, 0.5);
    makeStepper(ui->eepromFrame, "webPolesStepper", ui->motorPolesSlider,
                ui->motorPolesLCD);
    makeStepper(ui->eepromFrame, "webStartupStepper", ui->startupPowerSlider,
                ui->startupPowerLCD);
    makeStepper(ui->eepromFrame, "webPwmStepper", ui->pwmFreqSlider,
                ui->pwmFreqLCD);
    makeStepper(ui->eepromFrame, "webBeepStepper", ui->beepVolumeSlider,
                ui->beepVolumeLCD);
    makeStepper(ui->eepromFrame, "webSineRangeStepper", ui->sineStartupSlider,
                ui->sineRangeLCD);
    makeStepper(ui->eepromFrame, "webSinePowerStepper", ui->sineModePowerSlider,
                ui->sineLcd);
    makeStepper(ui->eepromFrame, "webRunningBrakeStepper",
                ui->runningBrakeStrength, ui->runningBrakeLcd);
    makeStepper(ui->eepromFrame, "webRocStepper", ui->maxRocSlider,
                ui->rocLineEdit, 0.1);
    makeStepper(ui->eepromFrame, "webDutyStepper", ui->minDutySlider,
                ui->minDutyLineEdit, 0.5);
    makeStepper(ui->eepromFrame, "webActiveBrakeStepper", ui->activeBrakeSlider,
                ui->activeBrakeLineEdit);

    auto syncStepper = [this](const char *name, QSlider *slider) {
        if (auto *stepper = findChild<QFrame *>(name)) {
            stepper->setEnabled(slider->isEnabled());
        }
    };
    const auto syncPwmStepper = [this, syncStepper] {
        syncStepper("webPwmStepper", ui->pwmFreqSlider);
    };
    const auto syncActiveBrakeStepper = [this, syncStepper] {
        syncStepper("webActiveBrakeStepper", ui->activeBrakeSlider);
    };
    syncPwmStepper();
    syncActiveBrakeStepper();
    connect(ui->varPWMCheckBox, &QCheckBox::stateChanged, this,
            [syncPwmStepper](int) { syncPwmStepper(); });
    connect(ui->activeBrakeCheckbox, &QCheckBox::stateChanged, this,
            [syncActiveBrakeStepper](int) { syncActiveBrakeStepper(); });

    // These options remain implemented for firmware compatibility, but are not
    // part of the clean web-style Settings view requested for everyday use.
    ui->rvCheckBox->hide();
    ui->biDirectionCheckbox->hide();
    ui->sinCheckBox->hide();
    ui->thirtymsTelemBox->hide();
    ui->hallSensorCheckbox->hide();
    ui->disableStickCalibCheckbox->hide();
    ui->autoPWM->hide();
    ui->signalComboBox->hide();
    ui->label_25->hide();
    ui->dragBrakeSlider->hide();
    ui->dragBrakeLCD->hide();
    ui->label_18->hide();
    ui->rcCarReverse->hide();
...
void Widget::arrangeWebLayout() {
    ...
    ui->escStatusLabel->hide();
    ui->StatusLabel->hide();
    ui->configFileInfo->hide();
    place(ui->ConnectedButton, qMax(640, headerWidth - 280), 27, 260, 33);

    const int width = ui->frame->width();
    place(findChild<QFrame *>("webActionSurface"), 0, 0, width, 47);

    const int actionY = 8;
    const int actionHeight = 31;
    const int actionGap = 7;
    int actionX = 10;
    const auto placeAction = [&](QPushButton *button, int buttonWidth) {
        place(button, actionX, actionY, buttonWidth, actionHeight);
        actionX += buttonWidth + actionGap;
    };
    placeAction(ui->sendButton, 108);
    placeAction(ui->writeEEPROM, 120);
    placeAction(ui->saveConfigButton, 106);
    placeAction(ui->loadConfigButton, 106);
    placeAction(findChild<QPushButton *>("webRestoreButton"), 130);
    placeAction(ui->disconnectButton, 96);

    const int serialX = qMax(actionX + 2, width - 240);
    place(ui->serialSelectorBox, serialX, 8, 112, 31);
    place(ui->checkBox_2, serialX + 118, 9, 108, 17);
    place(ui->OfflineCheckBox, serialX + 118, 25, 108, 17);
    ...
    place(ui->tabWidget, 0, 121, width, 489);

    const int tabWidth = qMax(840, ui->tab->width());
    place(ui->fourWayFrame, 7, 6, tabWidth - 14, 416);
    ui->fourWayFrame->show();
    place(ui->eepromFrame, 0, 6, tabWidth - 14, 365);
    ui->eepromFrame->show();
    ui->eepromFrame->raise();
    ...
    const int motorLabelWidth = 124;
    const int motorControlX = motorX + 142;
    const int motorControlWidth = qMax(94, motorWidth - 156);
    const int motorRows[] = {49, 80, 111, 142, 173, 204, 235, 266, 297};
    place(ui->timingLabel, motorLabelX, motorRows[0], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webTimingStepper"), motorControlX, motorRows[0], motorControlWidth, 24);
    place(ui->label_5, motorLabelX, motorRows[1], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webKvStepper"), motorControlX, motorRows[1], motorControlWidth, 24);
    place(ui->label_4, motorLabelX, motorRows[2], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webPolesStepper"), motorControlX, motorRows[2], motorControlWidth, 24);
    place(ui->label, motorLabelX, motorRows[3], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webStartupStepper"), motorControlX, motorRows[3], motorControlWidth, 24);
    place(ui->label_3, motorLabelX, motorRows[4], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webPwmStepper"), motorControlX, motorRows[4], motorControlWidth, 24);
    place(ui->label_13, motorLabelX, motorRows[5], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webBeepStepper"), motorControlX, motorRows[5], motorControlWidth, 24);
    place(ui->label_19, motorLabelX, motorRows[6], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webSineRangeStepper"), motorControlX, motorRows[6], motorControlWidth, 24);
    place(ui->label_20, motorLabelX, motorRows[7], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webSinePowerStepper"), motorControlX, motorRows[7], motorControlWidth, 24);
    place(ui->label_21, motorLabelX, motorRows[8], motorLabelWidth, 24);
    placeStepper(findChild<QFrame *>("webRunningBrakeStepper"), motorControlX, motorRows[8], motorControlWidth, 24);

    const int driveLabelX = driveX + 14;
    const int driveControlX = driveX + 138;
    const int driveControlWidth = qMax(78, driveWidth - 152);
    place(ui->label_31, driveLabelX, 49, 118, 24);
    placeStepper(findChild<QFrame *>("webRocStepper"), driveControlX, 49, driveControlWidth, 24);
    place(ui->label_32, driveLabelX, 80, 118, 24);
    placeStepper(findChild<QFrame *>("webDutyStepper"), driveControlX, 80, driveControlWidth, 24);
    ...
    placeStepper(findChild<QFrame *>("webActiveBrakeStepper"), driveControlX, 267, driveControlWidth, 24);
    ui->label_40->hide();

    const int inputWidth = qMax(820, ui->tab_4->width());
    place(ui->connectFrameInputPage, 10, 6, 315, 30);
    place(ui->inputservoFrame, 7, 6, inputWidth - 14, 378);
    ...
    const int flashWidth = qMax(820, ui->tab_2->width());
    place(ui->flashFourwayFrame, 10, 10, flashWidth - 20, 390);
    place(ui->MotorLabel, 14, 16, 44, 24);
    place(ui->escStatusLabel_2, 64, 16, 280, 24);
    place(ui->progressBar, 14, 58, 430, 24);
    place(ui->loadBinary, 14, 104, 126, 31);
    place(ui->writeBinary, 148, 104, 126, 31);
    place(ui->sendFirstEEPROM, 14, 340, 155, 31);
}
...
void Widget::connectSerial() {
    ...
    if (m_serial->open(QIODevice::ReadWrite)) {
        ...
        // This desktop UI controls exactly one ESC, on channel 0.
...
void Widget::on_disconnectButton_clicked() {
    ...
    closeSerialPort();
    four_way->ESC_connected = false;
    ui->escStatusLabel->setText("Not connected");
}
...
void Widget::readInitData() {
    ...
    if (data.size() < 9 || data.at(8) != char(0x30)) {
        four_way->ESC_connected = false;
        hideESCSettings(true);
        ui->escStatusLabel->setText("ESC not detected");
        return;
    }

    switch (static_cast<quint8>(data.at(4))) {
    case 0x2b:
        four_way->memory_divider_required_four = true;
        four_way->eeprom_address = 0x7e00;
        break;
    case 0x1f:
        four_way->memory_divider_required_four = false;
        four_way->eeprom_address = 0x7c00;
        break;
    case 0x35:
        four_way->memory_divider_required_four = false;
        four_way->eeprom_address = 0xF800;
        break;
    default:
        four_way->ESC_connected = false;
        hideESCSettings(true);
        ui->escStatusLabel->setText("Unsupported ESC flash code");
        return;
    }

    if (auto *address = findChild<QLabel *>("webInfoValue2")) {
        address->setText(
            QString("0x%1")
                .arg(QString::number(four_way->eeprom_address, 16).toUpper()));
    }

    four_way->ESC_connected = true;
    ui->escStatusLabel->setText(
        QString("EEPROM read at 0x%1")
            .arg(QString::number(four_way->eeprom_address, 16).toUpper()));
...
void Widget::on_writeBinary_clicked() {
    ...
    if(eeprom_buffer->size() != 0){
        ...
        if (four_way->ack_required == false) { // good ack received from esc
            ui->escStatusLabel->setText("Preparing firmware...");
        } else {
    ...
    if (sizeofBin == 0) {
        ui->escStatusLabel_2->setText("Firmware file is empty");
        return;
    }
    ui->escStatusLabel_2->setText("Flashing firmware...");
    uint16_t index = 0;
    ...
    for (int i = 0; i <= pages;
        ...
        for (int j = 0; j < 2048 / chunk_size; j++) { // 8 or 16 buffers per page
            ...
            if (four_way->ack_type == BAD_ACK) {
                ...
                ui->escStatusLabel_2->setText("Flash failed");
                return; //
            }
            ...
            const int progress = (index * 100) / sizeofBin;
            ui->progressBar->setValue(progress);
            ui->escStatusLabel_2->setText(
                QString("Flashing %1%").arg(progress));
            QApplication::processEvents();
            if (index >= sizeofBin) {
                ui->escStatusLabel_2->setText("Firmware flash complete");
                ui->progressBar->setValue(0);
                four_way->ack_required = true;
                ...
                if(eeprom_buffer->size() != 0) {
                    ...
                    if (four_way->ack_required == false) { // good ack received from esc
                        ui->escStatusLabel->setText("Settings saved");
                        writeMusic();
                        return;
                    } else {
                        ui->escStatusLabel->setText("Unable to set safety bit");
...
void Widget::endTimer(){
    ...
    if (connectMotor(four_way->connected_motor)) {
        ui->escStatusLabel->setText("ESC connected: Settings read OK ");
        return;
    } else {
        ui->escStatusLabel->setText("Retrying ESC connection... ");
        ui->StatusLabel->setText("Connecting");
    }
    if (connectMotor(four_way->connected_motor)) {
        ui->escStatusLabel->setText("Connected: Settings read OK ");
    } else {
        ui->escStatusLabel->setText("ESC connection failed");
    }
}
...
bool Widget::connectMotor(uint8_t motor) {
    ...
    if ((input_buffer->at(0) == (char)0x01)) {
        ...
        if (auto *eeprom = findChild<QLabel *>("webInfoValue1")) {
            eeprom->setText(QString("v%1.%2 - 48 bytes").arg(major, minor));
        }
...
void Widget::on_initMotor1_clicked() {
    ...
    if (connectMotor(0x00)) {
        ...
        ui->escStatusLabel->setText("Did not connect - retrying ");
        ui->StatusLabel->setText("Connecting");
    }
    if (connectMotor(0x00)) {
        ui->escStatusLabel->setText("Connected: Settings read OK ");
        ...
        ui->escStatusLabel->setText("Did not connect");
    }
    //  four_way->ack_required = true;
    //  writeData(four_way->makeFourWayCommand(0x37, 0x00));
    //  m_serial->waitForBytesWritten(200);
...
void Widget::on_writeEEPROM_clicked() {
    if (!m_serial || !m_serial->isOpen() || !four_way ||
        !four_way->ESC_connected) {
        ui->escStatusLabel->setText("ESC not connected");
        return;
    }

    if (!eeprom_buffer || eeprom_buffer->size() < 48) {
        ui->escStatusLabel->setText("EEPROM data unavailable");
        return;
    }

    four_way->ack_required = true;
    ui->escStatusLabel->setText("Saving settings...");
    QByteArray eeprom_out;
    for (int i = 0; i < 48; i++) {
        eeprom_out.append(eeprom_buffer->at(i));
    }
    ...
    if (four_way->direct) {
        ...
        if (four_way->ack_required == false) { // good ack received from esc
            ...
            ui->escStatusLabel->setText("Save failed (no ACK)");
        }
    }
}
...
void Widget::hide4wayButtons(bool b) {
    Q_UNUSED(b);
    // eepromFrame is a child of fourWayFrame: keep the parent visible.
    ui->fourWayFrame->show();
    ui->connectMotor->hide();
    ui->initMotor1->hide();
    ui->initMotor2->hide();
    ui->initMotor3->hide();
    ui->initMotor4->hide();
    ui->connectFrameInputPage->hide();
    ui->flashMotorsFrame->hide();
}
...
void Widget::on_saveConfigButton_clicked()
{
    ...
    QFileDialog::saveFileContent(eeprom_out, "ESC_CAR_V1.bin");
    qInfo("file written");
